...
def test_splice_page_shows_compact_endcard_summary_by_default(monkeypatch, tmp_path: Path) -> None:
    ...
    assert "预览落版" in _button_labels(app_test)
...
def test_preview_button_expands_video_only(monkeypatch, tmp_path: Path) -> None:
    app_test = _build_app_test(monkeypatch, tmp_path)

    app_test.run(timeout=10)
    app_test = next(button for button in app_test.button if button.label == "预览落版").click().run(timeout=10)
