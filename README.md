...
- **落版视频可上传管理**：拼接页顶部支持上传 `mp4` / `mov` 落版视频，上传后持久化保存，可通过媒体直链预览、可重新上传
...
“视频拼接”页默认使用 `assets/video/endcard.mp4` 作为初始落版视频。

如果业务同学需要替换落版，不再需要改代码或手动改环境变量，直接在页面顶部“落版视频管理”区上传 `mp4` / `mov` 文件即可。上传后的文件会持久化保存到本机，只保留一个“当前生效”的版本，页面刷新和应用重启后仍继续生效。

如果你确实需要在源码运行时指定另一份默认落版，仍可继续使用环境变量覆盖：
...
- 页面顶部提供紧凑的落版摘要条，可查看当前生效落版的来源、文件名和更新日期
- 点击“预览落版”会直接打开当前落版视频的媒体直链，交给浏览器原生播放器预览，不会再把主页面撑高
- 支持上传 `mp4`、`mov` 两种落版视频；点击“更换落版”会直接弹出系统文件选择器，选中文件后立即覆盖当前落版
- 上传后的落版视频会持久化保存到本机，页面刷新和应用重启后仍继续生效
- 如果还没有上传过新落版，系统默认使用 `assets/video/endcard.mp4`
- 重新上传新落版只影响后续新发起的拼接任务，不会中途切换已在处理中的批次
...
| 环境变量 | 默认值 | 说明 |
| --- | --- | --- |
| `SP_ENDCARD_PATH` | `assets/video/endcard.mp4` | 拼接页默认落版视频路径，源码运行时可覆盖 |
